...
import { dangKyTangCa } from '../../../../commons/api/instanceService';
...
class ShiftChangeTab extends BaseComponent {
  ...
  isValid = () => {
    ...
    if (this.content == '') {
      Toast.show('Bạn cần nhập lý do tăng ca.');
      return false;
    }
    ...
    if (this.isValid()) {
      let params = {
        ...
        shift: this.dataChangeShift.id == 2 ? this.shift : Number(this.shift),
			...
			try {
				let res = await dangKyTangCa(params);
      ...
      <View style={{flex: 1}}>
        <KeyboardAwareScrollView
          ...
          <Text style={[styles.styleTitle, {color: 'black', fontSize: 13}]}>
            {'Chọn thời gian bạn muốn xin tăng ca'}
          </Text>
          <View>
            {/*Select date*/}
            <InputView
...
const mapStateToProps = ({BaoCaoReducer}) => ({
  BaoCaoReducer
});
  ...
  return {
    ...
    createApplicationForChangeShift: (params) => {
      ...
      api.setUpLogin(dispatch, params);
		...
		dangKyTangCa: (params) => {
			api.dangKyTangCa(dispatch, params)
		}
  };
};
