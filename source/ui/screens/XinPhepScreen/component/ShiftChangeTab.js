...
class ShiftChangeTab extends BaseComponent {
  ...
  isValid = () => {
    if (this.dataChangeShift.id === 0) {
      Toast.show('Bạn cần chọn loại chuyển đổi ca.');
      return false;
    }
    if (this.startTime == '') {
      Toast.show('Bạn cần chọn thời gian bắt đầu.');
      return false;
    }
    if (this.endTime == '') {
      Toast.show('Bạn cần chọn thời gian kết thúc.');
      return false;
    }
    if (this.shift == null) {
    ...
    if (this.content == '') {
      Toast.show('Bạn cần nhập lý do xin chuyển đổi ca.');
      return false;
    }
  ...
  setGuiYeuCau() {
    if (this.isValid()) {
      let params = {
        time_start: `${configs.convertTimeDate(
          this.date,
          configs.FORMAT_DATE,
        )} ${this.startTime}`,
        time_end: `${configs.convertTimeDate(this.date, configs.FORMAT_DATE)} ${
          this.endTime
        }`,
      ...
      this.props.createApplicationForChangeShift(params);
    }
  }
