    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.props.DuyetNghiPhepReducer) {
            if (this.props.type == 1) {
                if (this.props.DuyetNghiPhepReducer.dataListDaDuyet !== prevProps.DuyetNghiPhepReducer.dataListDaDuyet) {
                    this.setState({
                        dataAll: this.props.DuyetNghiPhepReducer.dataListDaDuyet?.data
                    })
                }
            } else if (this.props.type == 2) {
                if (this.props.DuyetNghiPhepReducer.dataListTuChoi !== prevProps.DuyetNghiPhepReducer.dataListTuChoi) {
                    this.setState({
                        dataAll: this.props.DuyetNghiPhepReducer.dataListTuChoi?.data
                    })
                }
            }

            if (this.props.DuyetNghiPhepReducer.messageDelete && this.props.DuyetNghiPhepReducer.messageDelete !== prevProps.DuyetNghiPhepReducer.messageDelete) {
                setTimeout(() => {
                    Alert.alert(
                        configs.NAME_APP,
                        this.props.DuyetNghiPhepReducer.messageDelete.message,
                        [
                            {
                                text: 'Đồng ý', onPress: () => {
